  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok && data.success) {
        ...
        setError(data.error || (isAr ? "ØØ¯Ø« Ø®Ø·Ø£ Ù…Ø§ØŒ ØØ§ÙˆÙ„ Ù…Ø±Ø© Ø£Ø®Ø±Ù‰" : "An error occurred, please try again."));
      }
    } catch {
      setError(isAr ? "ÙØ´Ù„ Ø§Ù„Ø§ØªØµØ§Ù„ Ø¨Ø§Ù„Ø®Ø§Ø¯Ù…" : "Failed to connect to the server.");
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="bg-white p-8 rounded-xl shadow-sm border border-gray-100 max-w-md w-full">
        ...
        {success ? (
          <div className="mb-4 p-4 bg-green-50 text-green-700 rounded-lg text-sm text-center">
            {isAr 
              ? "ØªÙ… Ø¥Ø±Ø³Ø§Ù„ Ø±Ø§Ø¨Ø· Ø¥Ø¹Ø§Ø¯Ø© ØªØ¹ÙŠÙŠÙ† ÙƒÙ„Ù…Ø© Ø§Ù„Ù…Ø±ÙˆØ± Ø¥Ù„Ù‰ Ø¨Ø±ÙŠØ¯Ùƒ Ø§Ù„Ø¥Ù„ÙƒØªØ±ÙˆÙ†ÙŠ. ÙŠØ±Ø¬Ù‰ Ø§Ù„ØªØÙ‚Ù‚ Ù…Ù† ØµÙ†Ø¯ÙˆÙ‚ Ø§Ù„ÙˆØ§Ø±Ø¯." 
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            >
              {loading ? (isAr ? "Ø¬Ø§Ø±ÙŠ Ø§Ù„Ø¥Ø±Ø³Ø§Ù„..." : "Sending...") : (isAr ? "Ø¥Ø±Ø³Ø§Ù„ Ø±Ø§Ø¨Ø· Ø§Ù„Ø¥Ø¹Ø§Ø¯Ø©" : "Send Reset Link")}
            </button>
          </form>
        )}
        ...
        <p className="mt-6 text-center text-sm text-gray-600">
          ...
          >
            {isAr ? "Ø§Ù„Ø¹ÙˆØ¯ة Ø¥Ù„Ù‰ ØªØ³Ø¬ÙŠÙ„ Ø§Ù„Ø¯Ø®ÙˆÙ„" : "Back to Sign In"}
          </Link>
        </p>
      </div>
    </div>
